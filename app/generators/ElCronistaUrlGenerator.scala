...
class ElCronistaUrlGenerator extends BasicUrlGenerator{
  ...
  override def getGoogleSearchRegisters(query: String): List[String] = {
    ...
    try {
      ...
      println("Exited CronistaUrlGenerator without exception")
    } catch {
    ...
    override def cleanUrlDomain(url: String): String = {
      val regex : Regex = "(http|https)://www.cronista.com/(.+?)(?=.html).html".r
...
/**
  ...
  *     ElCronistaUrlGeneratorObject.search(Option(x), Option("Universidad Austral")).map((x,_)))
  *     println("URL: \n")
